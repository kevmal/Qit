...
namespace Qit.ProviderImplementation.ProvidedTypes
    ...
    type ProvidedTypesContext(referencedAssemblyPaths: string list, assemblyReplacementMap: (string*string) list, sourceAssemblies: Assembly list) as this =
        ...
        and convCodeToTgt (codeFun: Expr list -> Expr, isStatic, isCtor, parameters: ProvidedParameter[], isGenerated) = 
            (fun argsT -> 
                ...
                let paramNames = 
                    ...
                    [| if (not isStatic || isCtor) && (not isCtor || isGenerated) then yield "this"
        ...
        static member Create (config: TypeProviderConfig, assemblyReplacementMap, sourceAssemblies : Assembly list) =
            ...
            let referencedAssemblyPaths = config.ReferencedAssemblies |> Array.toList |> List.append (sourceAssemblies |> List.map (fun (x : Assembly) -> x.Location)) |> List.distinct
...
namespace Qit.ProviderImplementation.ProvidedTypes
    ...
    type CodeGenerator(assemblyMainModule: ILModuleBuilder, 
        ...
        and emitExpr expectedState expr =
            ...
            | TypeOf(None, [t1], []) ->
                emitExpr expectedState (Expr.Value(t1))
    ...
    type AssemblyCompiler(targetAssembly: ProvidedAssembly, context: ProvidedTypesContext, ?loadGeneratedTypesFromFile) =

        let loadGeneratedTypesFromFile = defaultArg loadGeneratedTypesFromFile false
        ...
        member this.Compile(isHostedExecution) =
            ...
            if isHostedExecution then 
                let realTargetAssembly = 
                    if loadGeneratedTypesFromFile then 
...
namespace Qit.ProviderImplementation.ProvidedTypes
    ...
    type TypeProviderForNamespaces(config: TypeProviderConfig, namespacesAndTypes: list<(string * list<ProvidedTypeDefinition>)>, assemblyReplacementMap: (string*string) list, sourceAssemblies: Assembly list, addDefaultProbingLocation: bool) as this =
        ...
        member val LoadGeneratedTypesFromFile = false with get,set
        ...
        interface ITypeProvider with
            ...
            member this.GetGeneratedAssemblyContents(assembly:Assembly) =
                ...
                | _ ->
                    let bytes = 
                        match assembly with 
                        | :? ProvidedAssembly as targetAssembly -> AssemblyCompiler(targetAssembly, ctxt, this.LoadGeneratedTypesFromFile).Compile(config.IsHostedExecution)
