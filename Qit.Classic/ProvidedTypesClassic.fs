...
namespace ProviderImplementation2.ProvidedTypes
...
type CodeGenerator(assemblyMainModule: ModuleBuilder, uniqueLambdaTypeName,
    ...
    and emitExpr expectedState expr =
        ...
        | Var v ->
            ...
            | false, _ ->
                failwithf "unknown parameter/field %A" v
        ...
        | DefaultValue (t) ->
            let lb = ilg.DeclareLocal (transType t)
            ilg.Emit(OpCodes.Ldloca_S,lb.LocalIndex)
            ilg.Emit(OpCodes.Initobj, t)
        ...
        | NewObject (ctor,args) ->
            ...
            popIfEmptyExpected expectedState
        | Value (obj, ty) when ty = typeof<Expr> -> 
            let str = 
                let bf = System.Runtime.Serialization.Formatters.Binary.BinaryFormatter()
                use ms = new MemoryStream()
                bf.Serialize(ms, obj)
                ms.ToArray() |> Convert.ToBase64String
            emitExpr expectedState 
                <@@
                    using (new MemoryStream(Convert.FromBase64String str))
                        (fun ms ->
                            let bf = System.Runtime.Serialization.Formatters.Binary.BinaryFormatter()
                            bf.Deserialize(ms) :?> Expr)
                @@>
        | Value (obj, ty) when ty = typeof<bigint> -> 
            let i = obj :?> bigint
            if i < bigint Int64.MaxValue && i > bigint Int64.MinValue then 
                let j = int64 i
                emitExpr expectedState <@@ Numerics.BigInteger(j) @@>
            else
                let a = i.ToByteArray()
                emitExpr expectedState <@@ Numerics.BigInteger(a) @@>
        | Value (obj, _ty) ->
            let rec emitC (v:obj) =
                match v with
...
type ProvidedConstructor(parameters : ProvidedParameter list) =
    ...
    member __.InvokeCode
        with set (q:Expr list -> Expr) =
            ...
            | Some _ -> invokeCode <- Some q//failwith (sprintf "ProvidedConstructor: code already given for '%s'" (nameText()))
        and get() = invokeCode.Value
...
type ProvidedMethod(methodName: string, parameters: ProvidedParameter list, returnType: Type) =
    ...
    member __.InvokeCode
        with set  (q:Expr list -> Expr) =
            match invokeCode with
            | None -> invokeCode <- Some q
            | Some _ -> invokeCode <- Some q//failwith (sprintf "ProvidedConstructor: code already given for %s on type %s" methodName (if declaringType=null then "<not yet known type>" else declaringType.FullName))
        and get() = invokeCode.Value
...
type ProvidedField(fieldName:string,fieldType:Type) =
    ...
    let customAttributesImpl = CustomAttributesImpl()
    let mutable fieldAttrs = FieldAttributes.Public
    member __.AddXmlDocComputed xmlDocFunction            = customAttributesImpl.AddXmlDocComputed xmlDocFunction
    member __.AddXmlDocDelayed xmlDocFunction             = customAttributesImpl.AddXmlDocDelayed xmlDocFunction
    member __.AddXmlDoc xmlDoc                            = customAttributesImpl.AddXmlDoc xmlDoc
    member __.AddObsoleteAttribute (message,?isError)     = customAttributesImpl.AddObsolete (message,defaultArg isError false)
    member __.AddDefinitionLocation(line,column,filePath) = customAttributesImpl.AddDefinitionLocation(line, column, filePath)
    member __.GetCustomAttributesDataImpl() = customAttributesImpl.GetCustomAttributesData()
    member __.AddCustomAttributesData(x) = customAttributesImpl.AddCustomAttribute(x)
#if FX_NO_CUSTOMATTRIBUTEDATA
#else
    override __.GetCustomAttributesData()                 = customAttributesImpl.GetCustomAttributesData()
#endif
    ...
    override __.ReflectedType                                     = notRequired "ReflectedType" fieldName //declaringType |> nonNull "ProvidedField.DeclaringType"
    override __.GetCustomAttributes(_inherit)                  =  [||]//notRequired "GetCustomAttributes" fieldName
    override __.GetCustomAttributes(_attributeType, _inherit)   = [||]//notRequired "GetCustomAttributes" fieldName
    override __.IsDefined(_attributeType, _inherit)             = notRequired "IsDefined" fieldName
        ...
        iterateTypes (fun  tb ptd ->
            ...
            for minfo in ptd.GetMethods(ALL) do
              ...
              | :? ProvidedMethod as pminfo   ->
                ...
                //printfn "ProvidedMethod %A" (pminfo.Name)
